...
func (cli *CLI) PrintHelp() {
	...
	commands := []struct {
		...
		{"builtins", "Show all builtin functions with examples"},
...
// Custom purple background highlighter that properly resets colors
func HighlightPurple(text, keyword string) string {
	...
	escaped := regexp.QuoteMeta(trimmedKeyword)
	re := regexp.MustCompile("(?i)" + escaped)

	// Use a custom color for purple background (HiMagenta bg + white fg) and ALWAYS reset after
	highlight := color.New(color.BgHiMagenta, color.FgHiWhite, color.Bold)

	return re.ReplaceAllStringFunc(text, func(match string) string {
		// Apply highlight and immediately reset to avoid bleeding
		return highlight.SprintFunc()(match) + color.New(color.Reset).Sprint("")
	})
}
